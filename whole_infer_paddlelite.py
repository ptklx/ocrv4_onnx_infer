...
def create_predictor_paddle_lite(model_dir):
    model_file_path = model_dir
    if not os.path.exists(model_file_path):
        raise ValueError("not find model file path {}".format(
            model_file_path))
    ...
    return predictor, input_tensor, None, None
...
class TextRecognizer(object):
    def __init__(self, model_dir,rec_char_dict_path,batch_num=6):
        ...
        # self.predictor, self.input_tensor, self.output_tensors, self.config = create_predictor(model_dir)
        #paddle_lite
        self.predictor, self.input_tensor, self.output_tensors, self.config = create_predictor_paddle_lite(model_dir)
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            self.predictor.run()
            outputs = self.predictor.get_output(0)
            preds = outputs.numpy()
            infer_et = time.time()
            print("rec cost time ", infer_et-infer_st)
...
if __name__ == "__main__":
    ...
    det_model_dir = r"D:\file\ocr_detect\PaddleOCR\paddle_weight\ch-pp-ocrv4-server\ch_PP-OCRv4_det_server_infer\model.onnx"
    # rec_model_dir=r"D:\file\ocr_detect\PaddleOCR\paddle_weight\ch-pp-ocrv4-server\ch_PP-OCRv4_rec_server_infer\ocrv4_rec.onnx"
    rec_model_dir=r"D:\file\ocr_detect\PaddleOCR\paddle_weight\ocrv4_nb\ocrv4_opt_rec_x86.nb"
